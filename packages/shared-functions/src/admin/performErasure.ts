...
/**
 * The erasure body proper — what deleting a member actually removes, with no
 * view on WHO asked for it.
 *
 ...
 * Auth account.
 *
 * Split out of `eraseUserAccount` (`deleteUser.ts`, issue #421) so that
 * function can wrap this call in a marker-cleanup/alert try/catch without
 * re-indenting several hundred lines, and so a unit test can mock THIS module
 * to pin that wrapping without staging a genuine mid-pipeline failure against
 * the emulator (see `__tests__/eraseUserAccountFailure.test.ts`).
 * `userRef`/`userData` are the guard's own read, passed through rather than
 * re-fetched.
 *
