/**
 * Europe/Paris wall-clock helpers — the ONE implementation (issue #309).
 *
 * Appointment/session `date` and `startTime` strings are wall-clock times
 * in Europe/Paris: the web apps write and display them as-is for their
 * Paris-based users. Cloud Functions run in UTC (and dev machines run in
 * whatever the local zone is), so parsing them with
 ...
 *
 * Pure Intl code with no runtime dependencies, so it lives here in the
 * leaf package: `@ejm/shared-functions/scheduled/parisTime.ts` re-exports
 * it for the sit/study crons and callables, and `@ejm/do-core`'s expiry
 * maths imports it directly (do-core cannot depend on shared-functions).
 * Import-identity tests in both consumers pin them to this module.
 */
