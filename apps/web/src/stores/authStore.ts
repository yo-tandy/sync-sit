 ...
 * stale cached epoch must never LOWER what login() already armed. The
 * forward-only max makes the CAPTURE commutative; the ordering of login()'s
 * capture vs the watcher's first server snapshot is handled separately (the
 * watcher suppresses enforcement on fresh sign-ins until its first server
 * snapshot arms — see the attach comment). Safe to keep maxing across
 * sessions of the SAME uid (the server epoch only ever moves forward); a
 * DIFFERENT uid replaces outright.
 */
