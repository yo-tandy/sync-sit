...
// Mirrors apps/study-web/src/lib/pushNotifications.ts with one deliberate
// change: do registrations are written to the SIBLING `fcmTokensDo` array —
// never `fcmTokens` (sit's legacy array) or `fcmTokensStudy`. The three apps
// are separate PWA installs on separate origins, so their FCM tokens must
// not mix — sendPushNotification picks the array by its `app` parameter
// (issue #168 Phase 1 pattern; fcmTokensDo per issue #297 / sync-do plan
// §13 PR9).

const VAPID_KEY = import.meta.env.VITE_FIREBASE_VAPID_KEY;
const PROMPTED_KEY = 'syncdo_push_prompted';
...
export async function requestPushPermission(userId: string): Promise<string | null> {
  ...
  try {
    ...
    // Save token to the user's DO token array (never `fcmTokens` or
    // `fcmTokensStudy`)
    await updateDoc(doc(db, 'users', userId), {
      fcmTokensDo: arrayUnion(token),
    });
...
export async function setupForegroundMessages(onNotification: (title: string, body: string) => void): Promise<() => void> {
  ...
  return onMessage(msg, (payload) => {
    const title = payload.notification?.title || payload.data?.title || 'Sync/Do';
