 ...
 * AppSwitchBarMount.test.tsx's suite this renders the REAL
 * `DoerAppBar`/`FamilyAppBar` (not mocked away), because the inline switcher
 * lives inside them. jsdom evaluates no media queries, so both halves of the
 * pair are always in the DOM at once; the responsive CONTRACT is the
 * classes, not visibility.
 */
...
describe('exactly one app-switch entry point per breakpoint (#417, plan Q9)', () => {
  ...
  it.each([
    ['DoerLayout', <DoerLayout key="d" />, '/doer'],
