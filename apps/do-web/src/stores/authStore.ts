// Auth store for apps/do-web, scaffolded from apps/study-web's (same
// cross-app session coherence: issue #181's sessionEpoch enforcement works
// identically in all three apps because they share one Auth instance). The
// only deltas: the user doc is typed as shared-core's User (no do-specific
// doc type exists yet — profiles.doer arrives at plan §13 PR4), and there is
// no push-token unregistration (do-web ships no push until PR9).
...
function epochMillisOf(data: User | null): number {
