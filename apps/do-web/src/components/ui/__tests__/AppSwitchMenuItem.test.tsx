...
describe('AppSwitchMenuItem (do → sit / do → study)', () => {
  afterEach(async () => {
  ...
  it('mints a handoff code and navigates to SIT with the code in the URL FRAGMENT', async () => {
    h.callable.mockResolvedValue({ data: { code: 'abc+/=' } });
    renderWithProviders(<AppSwitchMenuItem target="sit" />);
  ...
  it('navigates to STUDY when that is the target', async () => {
    h.callable.mockResolvedValue({ data: { code: 'xyz' } });
    renderWithProviders(<AppSwitchMenuItem target="study" />);

    fireEvent.click(screen.getByRole('button', { name: /open sync-study/i }));

    await waitFor(() => expect(h.assign).toHaveBeenCalledTimes(1));
    expect(h.assign).toHaveBeenCalledWith('https://sync-study-app.web.app/handoff#code=xyz&lang=en');
  });
