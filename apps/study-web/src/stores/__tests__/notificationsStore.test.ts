...
describe('notificationsStore (study)', () => {
  beforeEach(() => reset());
  ...
  it('lists only study-visible types and counts unread among them (sit-only types never light the bell)', () => {
    watchNotifications('u1');
    h.listeners[0].next(
      snapOf([
        notifDoc('a', 'study_contact_request', false),
        // Sit-world types: must NOT be listed nor counted in study.
        notifDoc('b', 'new_request', false),
        notifDoc('c', 'reference_received', false),
        // Guardian type: visible in BOTH apps.
        notifDoc('d', 'supervision_request', false),
        // Read row: listed but not counted.
        notifDoc('e', 'study_session_confirmed', true),
      ]),
    );
  ...
  it('markRead writes ONLY { read: true } to the tapped doc and drops the count', async () => {
    watchNotifications('u1');
    h.listeners[0].next(snapOf([notifDoc('a', 'study_contact_request', false)]));
  ...
  it('markRead on an already-read row writes nothing', async () => {
    watchNotifications('u1');
    h.listeners[0].next(snapOf([notifDoc('a', 'study_contact_request', true)]));
    await useNotificationsStore.getState().markRead('a');
