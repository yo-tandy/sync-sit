...
describe('CreateKidInvitePage', () => {
  beforeEach(() => reset());

  it('links each consent to its document', () => {
    renderWithProviders(<CreateKidInvitePage />);
  ...
  it('renders one identical success screen regardless of the backend branch', async () => {
    const renderToSuccess = async (resolution: Record<string, unknown>) => {
      ...
      const { container, unmount } = renderWithProviders(<CreateKidInvitePage />);
      fillIdentity();
      checkConsents();
      fireEvent.click(submitButton());
  ...
  it('explains the uniform screen and never reveals account existence', async () => {
    ...
    expect(screen.getByText(/we don't reveal whether an account already exists/i)).toBeInTheDocument();
