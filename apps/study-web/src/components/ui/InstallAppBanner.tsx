...
import { isRunningAsPWA } from '@ejm/shared-core';
import { db } from '@/config/firebase';
import { useAuthStore } from '@/stores/authStore';
import { Card, DownloadIcon } from '@ejm/shared-ui';
 ...
 *
 * Mirrors apps/web/src/components/ui/InstallAppBanner.tsx, with a STUDY
 * dismissal field: sit and study are separate installs on separate origins,
 * so dismissing the sit banner must not hide the study one (and vice versa).
 */
  ...
  const handleDismiss = async () => {
    ...
    try {
      await updateDoc(doc(db, 'users', uid), {
        dismissedPwaInstallBannerStudy: true,
