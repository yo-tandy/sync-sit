...
function MenuItem({ icon, label, to, onNavigate }: { icon: React.ReactNode; label: string; to: string; onNavigate?: () => void }) {
  return (
...
/**
 * Sync-study version of EnrollmentAppBar. Mirrors the sync-sit chrome
 * (sticky branded bar + hamburger menu) so the post-Step-Password phase
 * of tutor enrollment matches babysitter's visual treatment.
 *
 * Differences from sync-sit:
 * - No Sign Out item: tutor account doesn't exist until the final Prefs
 *   step calls enrollTutor, so there's nothing to log out of.
 */
