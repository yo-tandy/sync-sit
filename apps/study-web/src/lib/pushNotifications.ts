...
// Mirrors apps/web/src/lib/pushNotifications.ts with one deliberate change:
// study registrations are written to the SIBLING `fcmTokensStudy` array, never
// the legacy `fcmTokens` array (which stays sit's). The two apps are separate
// PWA installs on separate origins, so their FCM tokens must not mix —
// sendPushNotification picks the array by its `app` parameter (issue #168
// Phase 1).

const VAPID_KEY = import.meta.env.VITE_FIREBASE_VAPID_KEY;
const PROMPTED_KEY = 'syncstudy_push_prompted';
...
export async function requestPushPermission(userId: string): Promise<string | null> {
  ...
  try {
    ...
    // Save token to the user's STUDY token array (never the sit `fcmTokens`)
    await updateDoc(doc(db, 'users', userId), {
      fcmTokensStudy: arrayUnion(token),
    });
...
export async function removePushToken(userId: string): Promise<void> {
  ...
  try {
    ...
    if (token) {
      await updateDoc(doc(db, 'users', userId), {
        fcmTokensStudy: arrayRemove(token),
      });
...
export async function setupForegroundMessages(onNotification: (title: string, body: string) => void): Promise<() => void> {
  ...
  return onMessage(msg, (payload) => {
    const title = payload.notification?.title || payload.data?.title || 'Sync/Study';
