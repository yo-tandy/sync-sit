...
- `pnpm test:integration:laneN` builds shared packages + both functions codebases first (`pnpm build:emulator-deps`, issue #413), so a missing or stale functions dist no longer false-fails every callable test. This includes `@ejm/do-core`: without it the sit functions codebase fails to LOAD entirely, so `searchBabysitters` is silently absent and the search UI returns a bare `internal` — a failure that looks like a bug in the feature under test rather than a missing build. If you invoke `firebase emulators:exec` by hand instead of through the lane script, build first: `pnpm build:emulator-deps` (equivalently `pnpm -r --filter './packages/**' build && pnpm build:functions && pnpm build:study-functions`).
...
- The `build:emulator-deps` build step every lane script now runs first (issue #413) writes to `packages/*/dist` and `apps/{functions,study-functions}/dist` — shared, non-lane-scoped output, not something each lane gets its own copy of. Two lanes started close together each launch their own `tsc` processes against those same files at the same time. Stagger them instead: start the first lane and wait for its "All emulators ready" line (the build has finished by then — it runs before `firebase emulators:start`/`:exec`) before starting the second. If you know you're about to bring up multiple lanes, run `pnpm build:emulator-deps` once by hand first — every lane command still runs the build step itself, but doing it once up front, before firing off the lane commands, keeps that shared build off the critical path of getting each lane's emulators up.
