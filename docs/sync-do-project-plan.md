...
> A, the shared shell (decisions 21–25), since MOVED to
> `docs/platform-plan.md`.** Owner decisions in §2 and the platform plan §1 are
> settled; everything marked **OPEN** in §17 and the platform plan §7 is not.
>
> ⚠️ **If you are implementing this plan, read `docs/platform-plan.md` and
> §18 before PR2, PR4, PR7, PR8 and PR9.** It changes what do-web builds
> (no account page; a shared app-switch bar), records a notifications problem
...
- **Enrolling** requires a verified EJM identity — the gate that makes
  ...
  caller who either already holds a completed sit/study PROVIDER profile
...
**The suite-wide half of this appendix now lives in `docs/platform-plan.md`**
(issue #371). It covers the shared bottom bar that switches apps, the shared
account hub, the page-hero restructure, admin going neutral, the notification
preferences problem, and the domain consolidation — none of which is sync-do
work, and all of which changes sit and study at least as much as it changes do.

What stays here is the part that is genuinely about this ladder: which sync-do
PR each decision lands in.

### 18.1 Where each ladder PR is affected
