...
§18.9 is a later addition on a different subject: the owner has decided all
three apps move under one domain. It changes nothing in the ladder today, but
it decides the fate of the cross-origin handoff, so read it before assuming
`appHandoffCodes` is permanent.
...
- **Q12 — Paths or subdomains under the new domain** (18.9). Decisive: it
...
### 18.9 Domain consolidation — one domain, shape undecided
...
| Bottom bar (§18.2) | A real instant tab switch; no loading state | Keeps its pressed/loading state |
...
§18.2 (the bar's loading state), §18.8 Q10 (account hosting). None of them
