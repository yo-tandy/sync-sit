...
- **App-switch brand marks** — DONE (sync-do PR2, closes #302): the marks live in `packages/shared-ui/src/assets/` as direct PNG subpath exports (`@ejm/shared-ui/brand-marks/sync-{sit,study,do}.png`); the per-app byte-copies from PR #114 are deleted. Subpath (not barrel) exports keep each app's bundle free of the marks it doesn't use.
