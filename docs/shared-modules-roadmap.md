...
**Plan:** `docs/superpowers/plans/2026-06-13-portable-user-entity.md` — merged across PR #61 (package rename prep), PR #63 (Tier B: profiles schema, types only) and PR #72 (Tier D: writers/queries/rules flipped to `profiles.*` + migration callable).
...
- **Semantic error tokens for danger UI**: after the brand-token ramp (issue #118), danger/destructive UI still wears `brand-*` (historically red) — in sync-study those surfaces render brand blue. A semantic pass should move true-danger sites (destructive buttons, validation text, error banners) to the existing `error-*` tokens so destructive intent reads as red in both apps. Deliberate follow-up PR; the brand rename left the vocabulary clean for it.
