 ...
 * (same constraint documented on getVerificationDocument's and
 * createFamilyPhotoUploadUrl's integration suites), so a SUCCESSFUL call
 * here still fails — at the signing step, past every gate this suite
 * exists to test. "Passed authz" is proven the same way those suites prove
 * it: the call fails with something OTHER than the code the gate under
 * test would produce.
 *
 * The signer OPTIONS (action/version/contentType/expires) are pinned
 * separately, with Storage mocked, in
 * packages/shared-functions/src/verification/__tests__/createVerificationDocumentUploadUrl.test.ts.
 */
describe('createVerificationDocumentUploadUrl (authz + validation)', () => {
  let seed: SeedData;
  ...
  const VALID = { kind: 'identity', contentType: 'application/pdf', fileName: 'passport.pdf', sizeBytes: 1024 };
  ...
  it('rejects unauthenticated callers', async () => {
    await expect(
      callFunction('createVerificationDocumentUploadUrl', { familyId: seed.family1Id, ...VALID }),
    ).rejects.toMatchObject({ code: 'UNAUTHENTICATED' });
  });

  it('rejects a missing familyId', async () => {
    await expect(
      callFunction('createVerificationDocumentUploadUrl', { ...VALID }, ownFamilyParentToken),
  ...
  it("rejects a kind that isn't identity or enrollment", async () => {
    await expect(
      callFunction(
        ...
        { familyId: seed.family1Id, ...VALID, kind: 'ejm_enrollment' },
  ...
  it('rejects a missing fileName', async () => {
    await expect(
      callFunction(
        'createVerificationDocumentUploadUrl',
        { familyId: seed.family1Id, kind: 'identity', contentType: 'application/pdf', sizeBytes: 1024 },
        ownFamilyParentToken,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_ARGUMENT' });
  });
  ...
  it('rejects a missing sizeBytes', async () => {
    await expect(
      callFunction(
        'createVerificationDocumentUploadUrl',
        { familyId: seed.family1Id, kind: 'identity', contentType: 'application/pdf', fileName: 'passport.pdf' },
        ownFamilyParentToken,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_ARGUMENT' });
  });
  ...
  it('rejects contentType image/svg+xml (renderable/scriptable denylist)', async () => {
    await expect(
      callFunction(
        'createVerificationDocumentUploadUrl',
        { familyId: seed.family1Id, kind: 'identity', contentType: 'image/svg+xml', fileName: 'evil.svg', sizeBytes: 1024 },
        ownFamilyParentToken,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_ARGUMENT' });
  });
  ...
  it('rejects contentType text/html', async () => {
    await expect(
      callFunction(
        'createVerificationDocumentUploadUrl',
        { familyId: seed.family1Id, kind: 'identity', contentType: 'text/html', fileName: 'evil.html', sizeBytes: 1024 },
        ownFamilyParentToken,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_ARGUMENT' });
  });

  it('rejects a disallowed file extension even with a real document contentType', async () => {
    await expect(
      callFunction(
        'createVerificationDocumentUploadUrl',
        { familyId: seed.family1Id, kind: 'identity', contentType: 'application/pdf', fileName: 'passport.exe', sizeBytes: 1024 },
        ownFamilyParentToken,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_ARGUMENT' });
  });
  ...
  it('lets a family member past the authz gate for an enrollment document too', async () => {
    await expect(
      callFunction(
        'createVerificationDocumentUploadUrl',
        { familyId: seed.family1Id, ...VALID, kind: 'enrollment' },
        ownFamilyParentToken,
      ),
    ).rejects.toMatchObject({
      code: expect.not.stringMatching(/PERMISSION_DENIED|UNAUTHENTICATED|INVALID_ARGUMENT/),
